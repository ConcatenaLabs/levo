...
def run(d):
    ...
    code, r, h = _req(d.base, "OPTIONS", "/api/projects/helios/buy")
    ok.eq(h.get("Allow"), "POST, OPTIONS", "a route that takes POST alone does not claim HEAD")
    code, r, h = _req(d.base, "GET", "/api/projects/helios/buy")
    ok.eq(code, 405, "GET on it is refused")
    ok.eq(h.get("Allow"), "POST, OPTIONS", "with the same list OPTIONS gave")
    ...
    text = page.get("raw", "") if isinstance(page, dict) else str(page)
    ...
    ok.eq(code, 200, "robots.txt answers")
    ok.ok(rtext.startswith("User-agent: *\nAllow: /"), "and allows everything", rtext[:60])
    ok.ok("Sitemap: " in rtext and rtext.split("Sitemap: ", 1)[1].strip().endswith("/sitemap.xml"),
          "and names the sitemap", rtext)
    code, smap, h = _req(d.base, "GET", "/sitemap.xml")
    stext = smap.get("raw", "") if isinstance(smap, dict) else str(smap)
    ok.eq(code, 200, "sitemap.xml answers")
    ok.ok(h.get("Content-Type", "").startswith("application/xml"), "as XML", h.get("Content-Type"))
    ok.ok("<loc>" in stext and "/p/helios</loc>" in stext, "and names the sale's page", stext[:300])
    ok.ok("/projects</loc>" in stext and "/how-it-works</loc>" in stext, "and the board and the explainer")
    code, r = req("POST", "/api/projects/helios/flag", {"hidden": True}, token=issuer_tok)
    ok.eq(code, 200, "an operator takes the sale off the board")
    code, smap, h = _req(d.base, "GET", "/sitemap.xml")
    stext = smap.get("raw", "") if isinstance(smap, dict) else str(smap)
    ok.ok("/p/helios</loc>" not in stext, "and it leaves the sitemap with it")
    code, r = req("POST", "/api/projects/helios/flag", {"hidden": False}, token=issuer_tok)
    ...
    ok.ok("Helios Grid (HLX) \u00b7 open" in ftext,
          "titled by the sale's name, ticker and the board's word for its state", ftext[:400])
    ...
    # them took either.
    ok.section("wire in")
    t5 = {k: (str(v) if isinstance(v, int) else v) for k, v in dict(terms, min_lot=terms["min_lot"] + 11).items()}
