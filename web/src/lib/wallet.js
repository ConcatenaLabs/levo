...
export async function getUtxos(params) {
  return request('getUtxos', params || {})
}

export async function getAddress(params) {
  return request('getAddress', params || {})
}

// Relays an already-signed transaction. Levo never asks a wallet to sign a
// covenant spend: the sell leaf needs no signature, and the buyer's own inputs
// are signed by whatever holds their keys.
